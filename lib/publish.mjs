...
const PUBLISHERS = { xhs: publishXhs, douyin: publishDouyin };

// 用模板 visual + 内容渲染配图（setContent 截图，无临时文件）→ 写到 outPath。
export async function renderImage(browser, { brandTitle, title, paragraphs, footer, visual, outPath }) {
  const t = visual || {};
  const page = await browser.newPage({ viewport: { width: t.width || 1080, height: t.height || 1440 }, deviceScaleFactor: t.scale || 2 });
  try {
    await page.setContent(buildHtml({ brandTitle, title, paragraphs, footer, theme: t }), { waitUntil: 'networkidle' });
...
export async function publishDraft(draftId, targetIds, { mode = 'draft', onLog = () => {} } = {}) {
  ...
  const imgPath = resolve(UPLOADS_DIR, draft.image_path);
  ...
  for (const t of targets) {
    ...
    try {
      ...
      const res = await publish(handle.ctx, { imgPath, title: draft.title, body: draft.caption || '', hashtags, mode, titleMaxLen: titleMaxLen[t.platform] });
